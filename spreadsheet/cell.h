...
public:
    ...
    /*  TestClearCell требует, Sheet::GetCell для пустой (очищенной) ячейки возвращал nullptr.
    TestCellReferences требует, чтобы от пустой (упомянутой ранее) ячейки можно было вызвать GetReferencedCells,
    то есть чтобы Sheet::GetCell для пустой ячейки возвращал работающий указатель.
    Я не хочу бездумно удалять из таблицы очищенные ячейки, так как в них может оставаться информация о "верхних" связях.
    Придется создавать костыль, чтобы ячейка могла докладывать, была ли она недавно очищена, и Sheet::GetCell
    возвращал nullptr или интерфейс ячейки в зависимости от контекста.   */
